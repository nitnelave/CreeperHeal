...
public class CreeperCommandManager implements CommandExecutor
{
	...
	public boolean onCommand(CommandSender sender, Command command, String commandLabel, String[] args) {
		...
		{        //if it's just /ch, display help
			...
			WorldConfig current_world = CreeperConfig.world_config.get(args[args.length - 1]);   //the last argument can be a world
			...
			CreeperConfig.write();		//in case of a change of setting via a command, write it to the file
		}
		else {
			sender.sendMessage("/ch help");
			return true;
		...
		else
		{
			...
			if(args.length >2) {
				if(args[2].equalsIgnoreCase("all")) {
					...
					if(burnt)
						BurntBlockManager.forceReplaceBurnt(since, current_world);
					else
						ExplodedBlockManager.forceReplace(since, current_world);
				}
			}
			else {
				if(burnt)
	...
	{
		return CreeperPermissionManager.checkPermissions(player, true, nodes);
	}
