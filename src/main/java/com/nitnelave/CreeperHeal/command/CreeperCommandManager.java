...
public class CreeperCommandManager implements CommandExecutor {
    ...
    private void sendHelp(CommandSender sender) {
        ...
        else
        {
            if (admin)
            {
                sender.sendMessage(GREEN + "/ch reload :" + PURPLE
                                   + " reloads the config from the file.");
                sender.sendMessage(GREEN + "/ch (creeper|TNT|ghast|custom|fire) (on/off) (world) :"
                                   + PURPLE + " toggle replacement");
                sender.sendMessage(GREEN + "/ch [on|off] (world) :" + PURPLE
                                   + " toggle CH for this world.");
                sender.sendMessage(GREEN
                                   + "/ch (waitBeforeHeal|wbh) [seconds] :"
                                   + PURPLE
                                   + " Sets the interval before an explosion is replaced to x seconds");
                sender.sendMessage(GREEN + "/ch (waitBeforeHealBurnt|wbhb) [seconds] :" + PURPLE
                                   + " Same for a burnt block");
                sender.sendMessage(GREEN + "/ch (blockPerBlockInterval|bpbi) [ticks] :" + PURPLE
            ...
            if (heal)
            {
                ...
                sender.sendMessage(GREEN + "/ch healBurnt (world) :" + PURPLE
     ...
     */
    private void
        forceCmd(String[] args, String msg, CommandSender sender, WorldConfig currentWorld) {
        String cmd = args[0];
            ...
            if (args.length > 1)
            {
                ...
                if (target == null)
                {
                    player.sendMessage(ChatColor.RED
                                       + "This player is not online. /ch healNear <someone>");
                    return;
                }
